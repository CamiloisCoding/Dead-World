...
# ========================
def _get_map_editor():
    """Ensure the map_editor singleton is initialized before use."""
    import map_editor
    try:
        return map_editor.get_editor()
    except RuntimeError:
        return map_editor.init_editor(_game.rooms, MAP_LAYOUT_FILE)
...
def handle_keydown_map(event):
    ...
    if _game.node_naming:
        if event.key in (pygame.K_RETURN, pygame.K_ESCAPE):
            if event.key == pygame.K_RETURN and _game.selected_node_key and _game.node_name_input.strip():
                editor = _get_map_editor()
                ok = editor.rename_node(_game.selected_node_key, _game.node_name_input.strip())
                if ok:
                    if _game.current_room == _game.selected_node_key:
                        _game.current_room = _game.node_name_input.strip()
                    _game.selected_node_key = _game.node_name_input.strip()
            _game.node_naming = False
        elif event.key == pygame.K_BACKSPACE:
            _game.node_name_input = _game.node_name_input[:-1]
        elif event.unicode.isprintable() and len(_game.node_name_input) < 30:
            _game.node_name_input += event.unicode
    ...
    if event.key == pygame.K_UP:
        ...
        if _game.selected_node_key and _game.selected_node_key in _game.rooms:
            _game.node_naming = True
            _game.node_name_input = _game.selected_node_key
    elif event.key in (pygame.K_DELETE, pygame.K_x):
        if _game.selected_node_key:
            editor = _get_map_editor()
            if _game.selected_node_key == _game.current_room:
                 pass # Cannot delete current room
            else:
                 editor.remove_node(_game.selected_node_key)
                 _game.selected_node_key = None
        elif _game.selected_block_idx is not None and _game.selected_block_idx < len(_game.custom_blocks):
...
def handle_mouse_map_down(event):
    ...
    if event.button == 3:  # Right click
        if _game.context_menu_open:
            _game.context_menu_open = False
            return
            
        hit = _game.get_node_at_screen_pos(event.pos[0], event.pos[1], UNIT, cx, cy)
        if hit:
            _game.selected_node_key = hit
            _game.node_dragging = True
            _game.node_drag_key = hit
            _game.selected_block_idx = None
            
            # Open context menu
            _game.context_menu_open = True
            _game.context_menu_pos = event.pos
            _game.context_menu_node = hit
            _game.context_menu_items = [
                ("Umbenennen (F2)", "rename"),
                ("Löschen (Del)", "delete"),
            ]
        else:
            ...
            if blk_idx is not None:
                ...
                edge = _game.get_transition_at_screen_pos(event.pos[0], event.pos[1], UNIT, cx, cy)
                if edge:
                    _game.selected_block_idx = None
                    _game.selected_node_key = None
                    # Open context menu for edge
                    _game.context_menu_open = True
                    _game.context_menu_pos = event.pos
                    _game.context_menu_node = None
                    _game.context_menu_edge = edge
                    _game.context_menu_items = [
                        ("Raum einfügen", "split_edge"),
                    ]
                else:
                    _game.selected_block_idx = None
                    _game.selected_node_key = None
                    ...
                    _game.context_menu_pos = event.pos
                    _game.context_menu_node = None
                    _game.context_menu_edge = None
                    _game.context_menu_items = [
                        ("Neuer Raum (N)", "create_node"),
                    ]

    elif event.button == 1:  # Left click
        if _game.context_menu_open:
            # Check if clicked inside menu
            menu_x, menu_y = _game.context_menu_pos
            menu_width, row_height = 180, 30
            menu_height = len(_game.context_menu_items) * row_height
            if menu_x <= event.pos[0] <= menu_x + menu_width and menu_y <= event.pos[1] <= menu_y + menu_height:
                # Handle menu item click
                idx = (event.pos[1] - menu_y) // row_height
                if 0 <= idx < len(_game.context_menu_items):
                    action = _game.context_menu_items[idx][1]
                    if action == "rename":
                        _game.node_naming = True
                        _game.node_name_input = _game.context_menu_node
                        _game.selected_node_key = _game.context_menu_node
                    elif action == "delete":
                        editor = _get_map_editor()
                        if _game.context_menu_node != _game.current_room:
                            editor.remove_node(_game.context_menu_node)
                            if _game.selected_node_key == _game.context_menu_node:
                                _game.selected_node_key = None
                    elif action == "split_edge":
                        if _game.context_menu_edge:
                            from_room, to_room = _game.context_menu_edge
                            editor = _get_map_editor()
                            base_name = "neuer_raum"
                            new_name = base_name
                            counter = 1
                            while new_name in _game.rooms:
                                new_name = f"{base_name}_{counter}"
                                counter += 1
                                
                            ok = editor.insert_node_between(from_room, to_room, new_name)
                            if ok:
                                # Start naming the new node
                                _game.selected_node_key = new_name
                                _game.node_naming = True
                                _game.node_name_input = new_name
                                # Force transitions to rebuild in main thread
                        ...
                        # Generate at mouse pos
                        mx, my = _game.context_menu_pos
                        gx, gy = _game.screen_to_graph(mx, my, UNIT, cx, cy)
                        
                        editor.ensure_node(new_name)
                        editor.set_node_coords(new_name, gx, gy)
                        _game.GRAPH_LAYOUT[new_name] = (gx, gy)
